...
namespace LinqConsoleApp
{
    public class LinqSamples
    {
        ...
        */
        ...
        {
            var res = Emps.Join(Depts, dept => dept.Deptno, emp => emp.Deptno, (emp, dept) => new { emp, dept });
        }
        ...
        {
            var resL = Emps.Where(emp => emp.Job == "Frontend programmer").OrderByDescending(emp => emp.HireDate).FirstOrDefault();
        }
        ...
        {
            var res = Emps.Select(emp => emp.Salary).Aggregate(0, (max, salary) => salary > max ? salary : max);
        }
